...
void gpu_maxnorm_overall(Scalar *one, Scalar *result, int dim_local, int version, int mem_option);

template<typename Scalar>
float gpu_maxnorm_time(Scalar *one, Scalar *result, int dim_local, int runs, int version, int mem_option);
...
//------------------------------------------------------------------------------------------------/
    ...
    for(int r = 0;r<iteration;r++)
    {
        ...
        set_values_maxnorm(vector_host, vector_first, dimlocal);
        gpu_maxnorm_overall(vector_first, result_first, dimlocal, version_first, memory_option);
...
//------------------------------------------------------------------------------------------------/
    ...
    for (int r = 0; r<iteration; r++)
    {
        ...
        if (method != kernel_vs_cpu)
        {
            ...
            set_values_maxnorm(vector_host, vector_second, dimlocal);
            gpu_maxnorm_overall(vector_second, result_second, dimlocal, version_second, memory_option);
