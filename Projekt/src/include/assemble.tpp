			...
			for (size_t vtx_global = fron; vtx_global <= lron; vtx_global++)
			{
				...
				if (i <= nx*ny) //Boden
				{
					if (i == 1) //vorderer unterer linker Eckpunkt
					{
						...
						//zeile[vtx_global] = 3*11/38;
						//zeile[vtx_global+1] = -28/38;
						//zeile[vtx_global+2] = 17/38;
						//zeile[vtx_global+nx] = -28/38;
						//zeile[vtx_global+2*nx] = 17/38;
						//zeile[vtx_global+nx*ny] = -28/38;
						//zeile[vtx_global+2*nx*ny] = 17/38;
						...
						//zeile[vtx_global] += 3*1/sqrt(3)*3/2*h;
						//zeile[vtx_global+1] += 1/sqrt(3)*(-h)/2;
						//zeile[vtx_global+2] += 1/sqrt(3) *2*h;
						//zeile[vtx_global+nx] += 1/sqrt(3)*(-h)/2;
						//zeile[vtx_global+2*nx] += 1/sqrt(3) *2*h;
						//zeile[vtx_global+nx*ny] += 1/sqrt(3)*(-h)/2;
						//zeile[vtx_global+2*nx*ny] += 1/sqrt(3) *2*h;
					...
					else if (i == nx) //vorderer unterer rechter Eckpunkt
					{
						...
						//zeile[vtx_global] += 1*1/sqrt(3)*3/2*h;
						//zeile[vtx_global-1] += (-1)*1/sqrt(3)*(-h)/2;
						//zeile[vtx_global-2] += (-1)*1/sqrt(3) *2*h;
						//zeile[vtx_global+nx] += 1/sqrt(3)*(-h)/2;
						//zeile[vtx_global+2*nx] += 1/sqrt(3) *2*h;
						//zeile[vtx_global+nx*ny] += 1/sqrt(3)*(-h)/2;
						//zeile[vtx_global+2*nx*ny] += 1/sqrt(3) *2*h;
					...
					else if (i == nx*(ny - 1) + 1) //hinterer unterer linker Eckpunkt
					{
						...
						//zeile[vtx_global+1] += 1/sqrt(3)*(-h)/2;
						//zeile[vtx_global+2] += 1/sqrt(3) *2*h;
						//zeile[vtx_global-nx] += (-1)*1/sqrt(3)*(-h)/2;
						//zeile[vtx_global-2*nx] += (-1)*1/sqrt(3) *2*h;
						//zeile[vtx_global+nx*ny] += 1/sqrt(3)*(-h)/2;
						//zeile[vtx_global+2*nx*ny] += 1/sqrt(3) *2*h;
					}
					...
					else if (i < nx) //vordere untere Kante ohne Eckpunkt
					{
						...
						////zentraler Differezenquotient in x-Richtung möglich
						//zeile[vtx_global] = -2;
						//zeile[vtx_global+1] = 1;
						//zeile[vtx_global-1] = 1;
						////modifizierter Differenzenquotient in y/z-Richtung
						//zeile[vtx_global] += 2*11/38;
						//zeile[vtx_global+nx] = -28/38;
						//zeile[vtx_global+2*nx] = 17/38;
						//zeile[vtx_global+nx*ny] = -28/38;
						//zeile[vtx_global+2*nx*ny] = 17/38;
						...
						//zeile[vtx_global] += 2*1/sqrt(3)*3/2*h;
						//zeile[vtx_global+nx] += 1/sqrt(3)*(-h)/2;
						//zeile[vtx_global+2*nx] += 1/sqrt(3) *2*h;
						//zeile[vtx_global+nx*ny] += 1/sqrt(3)*(-h)/2;
						//zeile[vtx_global+2*nx*ny] += 1/sqrt(3) *2*h;
					...
					else if (i > nx*(ny - 1)) //hintere untere Kante ohne Eckpunkt
					{
						...
						//zeile[vtx_global] += 0*1/sqrt(3)*3/2*h;		
						//zeile[vtx_global-nx] += (-1)*1/sqrt(3)*(-h)/2;
						//zeile[vtx_global-2*nx] += (-1)*1/sqrt(3) *2*h;
						//zeile[vtx_global+nx*ny] += 1/sqrt(3)*(-h)/2;
						//zeile[vtx_global+2*nx*ny] += 1/sqrt(3) *2*h;
					...
					else // "innere" Punkte des Bodens
					{
						...
						//zeile[vtx_global] += 3/2*h;
						//zeile[vtx_global+nx*ny] += (-h)/2;
						//zeile[vtx_global+2*nx*ny] += 2*h;
				...
				else if (i > nx*ny*(nz - 1)) //Deckel
				{
					if (i == nx*ny*(nz - 1)+1) //vorderer oberer linker Eckpunkt
					{
						...
						//zeile[vtx_global+nx] += 1/sqrt(3)*(-h)/2;
						//zeile[vtx_global+2*nx] += 1/sqrt(3) *2*h;
						//zeile[vtx_global-nx*ny] += (-1)*1/sqrt(3)*(-h)/2;
						//zeile[vtx_global-2*nx*ny] += (-1)*1/sqrt(3) *2*h;
					}
					...
					else // "innere" Punkte des Deckels
					{
						...
						//zeile[vtx_global] += (-1)*3/2*h;
						//zeile[vtx_global-nx*ny] += (-1)*(-h)/2;
						//zeile[vtx_global-2*nx*ny] += (-1)*2*h;
				...
				else if (i % nx*ny <= nx) //vordere Seite, aber nicht Boden oder Deckel
				{
					...
					else //vordere "innere" Seite
					{
						...
						//zeile[vtx_global] += 3/2*h;
						//zeile[vtx_global+nx] += (-h)/2;
						//zeile[vtx_global+2*nx] += 2*h;
					}
				}
				...
				else if (i % nx*ny > nx*(ny - 1)) //hintere Seite, aber nicht Boden oder Deckel
				{
					...
					else // hintere "innere" Seite
					{
						...
						//zeile[vtx_global] += (-1)*3/2*h;
						//zeile[vtx_global-nx] += (-1)*(-h)/2;
						//zeile[vtx_global-2*nx] += (-1)*2*h;
					}
				}
				...
				else if (i % nx == 1) //linke Seite, aber nicht vordere/hintere Seite oder Boden/Deckel
				{
					...
					//zeile[vtx_global] += 3/2*h;
					//zeile[vtx_global+1] += (-h)/2;
					//zeile[vtx_global+2] += 2*h;
				}
				...
				else if (i % nx == 0) //rechte Seite, aber nicht vordere/hintere Seite oder Boden/Deckel
				{
					...
					//zeile[vtx_global] += (-1)*3/2*h;
					//zeile[vtx_global-1] += (-1)*(-h)/2;
					//zeile[vtx_global-2] += (-1)*2*h;
				}
