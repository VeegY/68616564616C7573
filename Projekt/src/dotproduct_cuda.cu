...
__global__ void kernel(type *vectorx, type *vectory, type *placehold, int dim_local)
{
...
template float invoke_gpu_time<float>(float *vecx, float *vecy, float *result, int dim, int runs);
template float invoke_gpu_time<double>(double *vecx, double *vecy, double *result, int dim, int runs);
