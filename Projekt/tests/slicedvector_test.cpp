...
#include <limits>
/*
*test fuer slicedvector
*KEIN unit-test
*
*/
int slicedvectortest()
{
    srand (static_cast <unsigned> (time(0)));
    const size_t N=100000;

    Icarus::SlicedVector<double> vec1(N), vec2(N), vec4(N);
    Icarus::SlicedVector<double> vec6(N), vec7(N), vec8(N);
    ...
    Icarus::SlicedVector<double> vec3(vec1); //test copy constructor
    vec2=vec1;
    ...
    {
        LOG_ERROR("L2norm2 failed; value: ",vec6.l2norm2(), "  reference value: ", l2norm2_glob, "  difference: ",l2norm2-vec6.l2norm2());
    }
